...
public:
    ...
    void contract_arches(int i)
    {
        ...
        if ((site1 != OPENING && site1 != CLOSING) || (site2 != OPENING && site2 != CLOSING))
        {
            return;
        }
    ...
    {
        int ip1 = (i + 1) % size;
        return (((*this)[i] == BOT_DEF && is_middle(ip1)) || ((*this)[ip1] == BOT_DEF && is_middle(i)) && nb_defects() - 2 >= min_defects);
    }

    // bool can_contract_defects(int i, int min_defects) {
    //     int ip1 = (i+1)%size;
    //     return ((((*this)[i] == BOT_DEF && is_middle(ip1)) \
    //          || ((*this)[ip1] && is_middle(i))) \
    ...
    {
        int ip1 = (i + 1) % size;
        return (can_contract_defects(i, min_defects) || (*this)[i] < DEFECT || (*this)[ip1] < DEFECT);
    }
