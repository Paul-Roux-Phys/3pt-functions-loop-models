...
RMatrix<Vec, int, int, V> insert_mid = insert_middle_operator;
...
LargeFloat compute_3pt_function(int half_size, int k1, int k2, int k3, int rs1, int rs2, int rs3, int starting_position, V P1 = 0, V P3 = 0)
{
    if (k2 == 0)
    {
        return abs(compute_2pt_function(half_size, k1, rs1, starting_position));
    }

    Vec end = end_state(k1, k2, k3, rs1, rs2, -rs3);
    // Vec end_no_parity = end_state(k1, k2, k3, rs1, rs2, -rs3);
    // Vec end;
    // multiply<Vec, int>(&end, proj_parity, &end_no_parity, 1);
    LargeFloat res;

    // else
    vp += bottom_state(k1, rs1, starting_position, P1);
    ...
    for (int i = 0; i < half_size; i++)
    {
        transfer(k3);
        // if (rs1 != 0)
        //     vp.mul<int, int, bool>(proj_Zk_rep_bot, k1, rs1, true);
        // vp.print();
        // vp.mul<int>(proj_parity, 1);
        // vp.print();
    }
    ...
    // vp.print(); 
    vp.mul<int, int, V>(insert_mid, k2, k3, P3);
    // vp.mul<int>(proj_parity, 1);
