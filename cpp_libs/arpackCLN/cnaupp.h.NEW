...
inline void naupp(ARint& ido, char bmat, ARint n, const std::string& which, ARint nev,
                  T& tol, T* resid, ARint ncv, T* v,
                  ARint ldv, ARint* iparam, ARint* ipntr, T* workd,
                  T* workl, ARint lworkl, ARint& info, int digits)
{
 ...
 // extern struct debugstruct debug;
  ...
  int ierr,j;
  static int bounds,ih,iq,ishift,iupd,iw,ldh,ldq;
  static int levec,mode,msglvl,mxiter,nb,nev0,next;
  static int np,ritzi,ritzr;
  
  //std::cout << "calling naupp with ido = " << ido << "\n";
  ...
  //  std::string tempstring("E");
    if(tol<=zero)
    {
//      std::cout << "calling dlamch \n";
      tol=dlamch<T>("E",digits);
//      std::cout << "back from dlamch \n";
    }
    ...
    /*-----------------------------*/
    /* Zero our internal workspace */
    /*-----------------------------*/
    
    for(j=0;j<=(3*ncv*ncv+6*ncv);j++)
      workl[j]=zero;
    ...
    /*-------------------------------------------------------*/
 //   std::cout << "Calling dnaup2 \n";
    
    dnaup2<T>(ido,bmat,n,which,nev0,np,tol,&resid[0],mode,iupd,ishift,
           mxiter,&v[1],ldv,&workl[ih],ldh,&workl[ritzr],&workl[ritzi],
           &workl[bounds],&workl[iq],ldq,&workl[iw],&ipntr[0],&workd[1],info,digits);
    ...
    iparam[3]=mxiter;
    iparam[5]=np;
    iparam[9]=timing.nopx;
    iparam[10]=timing.nbx;
    iparam[11]=timing.nrorth;
    ...
    if(msglvl>0)
    {
      ivout<T>(debug.logfil,1,&mxiter,debug.ndigit,"_naupd: Number of update iterations taken");
      ivout<T>(debug.logfil,1,&np,debug.ndigit,"_naupd: Number of wanted 'converged' Ritz values");
      dvout<T>(debug.logfil, np, &workl[ritzr], debug.ndigit, "_naupd: Real part of the final Ritz values");
      dvout<T>(debug.logfil, np, &workl[ritzi], debug.ndigit, "_naupd: Imaginary part of the final Ritz values");
      dvout<T>(debug.logfil, np, &workl[bounds], debug.ndigit, "_naupd: Associated Ritz estimates");
    }
    
    // unimplemented timing stuff
     second(timing.t1);
     timing.tnaupd = timing.t1-timing.t0 ;
...
/* Commented out by JLJ
     std::cout << "=============================================\n";
      std::cout << "= Nonsymmetric implicit Arnoldi update code =\n";
      std::cout << "= Version Number: 2.4++ =\n";
      std::cout << "= Version Date: today   =\n";
      std::cout << "=============================================\n";
      std::cout << "= Summary of timing statistics              =\n";
      std::cout << "=============================================\n";
 
      std::cout << "Total number update iterations             = " << mxiter << "\n";
      std::cout << "Total number of OP*x operations            = " << timing.nopx << "\n";
      std::cout << "Total number of B*x operations             = " << timing.nbx << "\n";
      std::cout << "Total number of reorthogonalization steps  = " << timing.nrorth << "\n";
      std::cout << "Total number of iterative refinement steps = " << timing.nitref << "\n";
      std::cout << "Total number of restart steps              = " << timing.nrstrt << "\n";
      std::cout << "Total time in user OP*x operation          = " << timing.tmvopx << "\n";
      std::cout << "Total time in user B*x operation           = " << timing.tmvbx << "\n";
      std::cout << "Total time in Arnoldi update routine       = " << timing.tnaupd << "\n\n"; 
*/
...
inline void naupp(ARint& ido, char bmat, ARint n, const std::string& which, ARint nev,
                  T& tol, T* resid, ARint ncv, T* v,
                  ARint ldv, ARint* iparam, ARint* ipntr, T* workd,
                  T* workl, ARint lworkl, ARint& info)
{
  const T one=1.0;
  const T zero=0.0;
 // extern struct debugstruct debug;
 ...
 //   std::cout << "Calling dnaup2 \n";
    ...
    dnaup2<T>(ido,bmat,n,which,nev0,np,tol,&resid[0],mode,iupd,ishift,
	   mxiter,&v[1],ldv,&workl[ih],ldh,&workl[ritzr],&workl[ritzi],
	   &workl[bounds],&workl[iq],ldq,&workl[iw],&ipntr[0],&workd[1],info);
