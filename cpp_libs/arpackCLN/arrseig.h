...
void ARrcStdEig<ARFLOAT,ARTYPE>::Copy(const ARrcStdEig<ARFLOAT,ARTYPE>& other)
{
  ...
  copy<ARFLOAT>(3*n+1,other.workd,1,workd,1);

  V         = new ARTYPE[n*ncv+1];     // V.
  copy<ARFLOAT>(n*ncv+1,other.V,1,V,1);

  if (newRes) {                        // resid.
    resid   = new ARTYPE[n];
    copy<ARFLOAT>(n,other.resid,1,resid,1);
  }
  else {
    resid   = other.resid;
  }
  ...
  if (newVec) {                        // EigVec.
    EigVec  = new ARTYPE[ValSize()*n];
    copy<ARFLOAT>(ValSize()*n,other.EigVec,1,EigVec,1);
  }
...
int ARrcStdEig<ARFLOAT, ARTYPE>::FindEigenvalues()
{
  ...
  if (!ValuesOK) {
    ...
    // next line is a temporary command to place eigenvalues in EigValR and EigValI 
    for(int i=0;i<nconv;i++)
    {
      EigValR[i]=workl[ipntr[6]+i];
      EigValI[i]=workl[ipntr[7]+i];
    }
  //  EigValR=&workl[ipntr[6]];
  //    std::cout << "Eigenvalue " << i << ":" << workl[ipntr[6]+i] << "\n";
